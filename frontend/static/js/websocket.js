    ...
    if (windowWidth < 700) multi = windowWidth / 700;
      ...
      if (data.message === "") {
        ...
        words.forEach((word) => {
          ...
          if (word === "VS") span.className = "announcement_VS";
          else if (word === "Victory") span.className = "announcement_victory";
      ...
      const announcements_victory = document.querySelector(
        ".announcement_victory"
      );
      ...
      if (data.array === "") {
        ...
        array.forEach((entry, index) => {
          ...
          player_name.setAttribute(
            "href",
            `/profile?profile=${player.username}`
          );
          if (index != 0) player_name.className = "text-white";
          else player_name.className = "text-red";
          td1.appendChild(avatar);
          td1.appendChild(player_name);
          if (index <= 2) {
            let crown_link;
            if (index == 0) crown_link = "/static/assets/gold_crown.png";
            else if (index == 1) crown_link = "/static/assets/argent_crown.png";
            else crown_link = "/static/assets/bronze_crown.png";
...
function checkWebsocketPage(page, queryString = "") {
  ...
  if (urls[0] === "chat" && urls[1]) {
    const id = urls[1];
    chatWebSocket(`wss://localhost/ws/${urls[0]}/${id}/`);
  } else if (urls[0] === "waiting") {
    const game_mode = urls[1];
    const queue_type = urls[2];
    waitingWebSocket(
      `wss://localhost/ws/${urls[0]}/${game_mode}/${queue_type}/`
    );
  } else if (urls[0] === "game") {
    const game = urls[1];
    const mode = urls[2];
    ...
    if (game === "pong")
      pongWebsocket(
        `wss://localhost/ws/${urls[0]}/${game}/${mode}/${id}/`,
        mode
      );
